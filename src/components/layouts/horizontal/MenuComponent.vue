                ...
                >
                  <template v-for="(subitem, index) of item.subItems">
                    ...
                    <div v-if="item.subItems.length > 11" :key="index">
                      <BRow v-if="index % 3 == 0">
                        <BCol lg="4">
                          <nuxt-link class="dropdown-item side-nav-link-ref" :to="subitem.link">{{
                            $t(item.subItems[index].label)
                          }}</nuxt-link>
                        </BCol>
                        <BCol v-if="item.subItems[index + 1]" lg="4">
                          <nuxt-link
                            ...
                            >{{ $t(item.subItems[index + 1].label) }}</nuxt-link
                          >
                        </BCol>
                        <BCol v-if="item.subItems[index + 2]" lg="4">
                          <nuxt-link
                            ...
                            >{{ $t(item.subItems[index + 2].label) }}</nuxt-link
                          >
                        </BCol>
                      </BRow>
                    </div>
...
<script>
import { horizontalMenu } from '../../layouts/utils/menu.js'
