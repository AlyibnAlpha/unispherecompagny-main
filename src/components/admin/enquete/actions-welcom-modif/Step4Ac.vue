  ...
  setup(props) {
    ...
    const deleteRow2 = async (index, indexs) => {
      if (fields.value[index].choices.length > 1) {
        if (fields.value[index].choices[indexs].label !== '') {
          ...
          store
            .setSurveyDataw(response.data.find((s) => s.id === store.idsurveyw))
            .catch((error) => {
    ...
    const deleteRow = async (index) => {
      if (fields.value[index].title !== '') {
        ...
        store.setSurveyDataw(response.data.welcome_surveys.find((s) => s.id === store.idsurveyw))
      } else {
        fields.value.splice(index, 1)
      }
    }
