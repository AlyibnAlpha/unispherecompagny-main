...
export default {
  ...
  setup() {
    ...
    const gets = async () => {
      if (store.surveyDataw && Object.keys(store.surveyDataw).length > 0) {
        orderData.value = store.surveyDataw
      } else {
        try {
          const response = await api.get('/admin/welcome-surveys')
          const found = response.data.welcome_surveys.find((item) => item.id === store.idsurveyw)
