...
export default {
  ...
  setup(props) {
      ...
      for (const option of optionl2.value) {
        ...
        try {
          const response = await api.get(
            `/surveys/${store.idsurvey}/question-groups/${groupId}/questions`,
          )
          ...
          console.error(`Erreur lors de la récupération des questions du groupe ${groupId}:`, error)
        } finally {
          loading.value = false
        }
      }
    ...
    const deleteRow = async (index) => {
      if (fields.value[index].title !== '') {
        ...
        if (role === import.meta.env.VITE_DEFAULT_ROLEB) {
          await api
            .delete(
              `/surveys/${store.idsurvey}/question-groups/${fields.value[index].group.id}/questions/${index}`,
            )
            .then(() => {
              Notify.create({
                type: 'success',
          ...
          await api
            .delete(
              `/admin/surveys/${store.idsurvey}/question-groups/${fields.value[index].group.id}/questions/${index}`,
            )
            .then(() => {
              Notify.create({
                type: 'success',
        ...
        try {
          ...
          if (role === import.meta.env.VITE_DEFAULT_ROLEB) {
            for (const t of add.value) {
              await api.post(`/surveys/${store.idsurvey}/question-groups/${t.group}/questions`, {
                title: t.title,
            ...
            for (const t of add.value) {
              await api.post(
                `/admin/surveys/${store.idsurvey}/question-groups/${t.group}/questions`,
                {
                  title: t.title,
                  description: t.description,
                  type: t.type,
      ...
      if (role === import.meta.env.VITE_DEFAULT_ROLEB) {
        for (const t of updates) {
          try {
            loadings.value = true
            await api.put(
              `/surveys/${store.idsurvey}/question-groups/${t.group}/questions/${t.id}`,
              {
                title: t.title,
                description: t.description,
                type: t.type,
        ...
        for (const t of updates) {
          try {
            loadings.value = true
            await api.put(
              `/admin/surveys/${store.idsurvey}/question-groups/${t.group}/questions/${t.id}`,
              {
                title: t.title,
                description: t.description,
                type: t.type,
    ...
    watch(
      ...
      () => {
        console.log('Step4Ac rafraîchi !')
        getss() // ta fonction de refresh interne
      },
    )
