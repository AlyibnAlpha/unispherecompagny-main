<template>
  ...
  <div class="mb-3 text-end">
    <BRow class="d-flex justify-content-between align-items-center">
      <BCol cols="auto"
        ><router-link class="btn btn-link text-muted" to="/admin/enquetes-mes">
  ...
  <div v-else>
    <BRow class="align-items-stretch">
      <BCol cols="8" order="1" class="h-100">
        <BRow>
          <BCol lg="12">
            <div id="stat-detail">
      ...
      <BCol cols="4" order="2" class="border-end d-flex flex-column p-1">
        <div class="text-center mb-4">
          <BCard
            class="shadow-lg border-0 p-1 rounded-4 position-relative overflow-hidden"
            style="transition: transform 0.3s; cursor: pointer"
            @mouseover="(e) => (e.currentTarget.style.transform = 'translateY(-2px) scale(1.02)')"
...
export default {
  ...
  setup() {
    ...
    const genderChartOptions = {
      ...
      plotOptions: {
        pie: {
          size: 150, // agrandir ou réduire (par défaut ~150-180)
        },
      },
      tooltip: {
        y: {
    ...
    const tableFields = [
      ...
      { key: 'percent', label: 'Pourcentage', sortable: true },
    ]
    ...
    const participantStats = computed(() => {
      ...
      return [
        { category: 'Total des participants', count: stats.totalParticipants, percent: '100%' },
        {
          ...
          percent: ((stats.totalResponded / stats.totalParticipants) * 100).toFixed(2) + '%',
        },
        {
          category: 'Total non répondus',
          count: stats.totalNotResponded,
          percent: ((stats.totalNotResponded / stats.totalParticipants) * 100).toFixed(2) + '%',
        },
        {
          category: 'Total notifier',
          count: stats.totalNotified,
          percent: ((stats.totalNotified / stats.totalParticipants) * 100).toFixed(2) + '%',
        },
        {
          category: 'Total non notifier',
          count: stats.totalNotNotified,
          percent: ((stats.totalNotNotified / stats.totalParticipants) * 100).toFixed(2) + '%',
        },
        {
          category: 'Total des participants qui ont pas commencés',
          count: stats.totalNotStarted,
          percent: ((stats.totalNotStarted / stats.totalParticipants) * 100).toFixed(2) + '%',
        },
        {
          category: 'Total des participants en progressions du sondage',
          count: stats.totalInProgress,
          percent: ((stats.totalInProgress / stats.totalParticipants) * 100).toFixed(2) + '%',
        },
        {
          category: 'Total des paarticipants qui sont complétés',
          count: stats.totalCompleted,
          percent: ((stats.totalCompleted / stats.totalParticipants) * 100).toFixed(2) + '%',
        },
      ]
    })
